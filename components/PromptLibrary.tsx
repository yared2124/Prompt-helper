...
export default function PromptLibrary({
  ...
  const handleExportJSON = () => {
    ...
    a.href = url;
    a.download = `promptcraft-backup-${Date.now()}.json`;
    a.click();
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      {/* Drawer Container */}
      <div className="relative ml-auto h-full w-full max-w-lg bg-white dark:bg-[#090b17] border-l border-slate-200 dark:border-white/10 shadow-2xl flex flex-col z-10 animate-slide-left">
        {/* Header */}
        <div className="px-5 py-4 border-b border-slate-200 dark:border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <BookmarkCheck size={20} className="text-indigo-600 dark:text-violet-400" />
            <h3 className="font-bold text-base text-slate-900 dark:text-white">
              Prompt Library
            </h3>
            <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 dark:bg-white/10 font-mono text-slate-600 dark:text-slate-300">
              {prompts.length}
            </span>
          </div>
        ...
        {/* Search & Filter Strip */}
        <div className="p-4 border-b border-slate-200 dark:border-white/10 space-y-2 bg-slate-50/50 dark:bg-white/[0.02]">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 rounded-xl text-xs bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-800 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:border-indigo-500"
            />
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {filteredPrompts.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              <BookmarkCheck size={40} className="text-slate-300 dark:text-slate-700 mb-3" />
              <p className="text-slate-600 dark:text-slate-400 font-medium text-sm">
                {searchQuery ? "No matching prompts found" : "No saved prompts yet"}
              </p>
              <p className="text-slate-400 dark:text-slate-600 text-xs mt-1 max-w-xs">
                {searchQuery
            ...
            filteredPrompts.map((p) => (
              <div
                key={p.id}
                className="p-3.5 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50/70 dark:bg-white/[0.02] hover:border-slate-300 dark:hover:border-white/20 transition-all space-y-2.5"
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md bg-indigo-100 dark:bg-violet-950/50 text-indigo-700 dark:text-violet-300">
                      {p.aiProvider}
                    </span>
                    <span className="text-[10px] px-2 py-0.5 rounded-md bg-slate-200 dark:bg-white/10 font-medium text-slate-700 dark:text-slate-300">
                      {p.domain}
                    </span>
                  </div>
                ...
                <p className="text-xs font-mono text-slate-800 dark:text-slate-200 line-clamp-3 leading-relaxed bg-white dark:bg-black/30 p-2.5 rounded-lg border border-slate-200/60 dark:border-white/5">
                  {p.enhancedPrompt}
                </p>
                ...
                <div className="flex items-center justify-between pt-1">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => onLoad(p)}
                      className="text-xs px-2.5 py-1.5 rounded-lg font-medium bg-indigo-50 dark:bg-violet-950/40 text-indigo-600 dark:text-violet-300 hover:bg-indigo-100 dark:hover:bg-violet-950/70 transition-colors"
                    >
                      Load into Editor
                    </button>
                  </div>
        ...
        {prompts.length > 0 && (
          <div className="p-3 border-t border-slate-200 dark:border-white/10 flex items-center justify-between bg-slate-50 dark:bg-white/[0.02]">
